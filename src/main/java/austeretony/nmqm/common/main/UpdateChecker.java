	...
	public void onPlayerLoggedIn(PlayerLoggedInEvent event) {
		if (NMQMDataLoader.isUpdateCheckeEnabled() && CommonReference.isOpped(event.player)) {
			if (this.compareVersions(NMQMMain.VERSION, availableVersion)) {	
	        	...
	        	event.player.sendMessage(updateMessage1.appendSibling(updateMessage2).appendSibling(updateMessage3));
	        	event.player.sendMessage(pageMessage1.appendSibling(pageMessage2).appendSibling(urlMessage));	        	
			}
		}
	}
