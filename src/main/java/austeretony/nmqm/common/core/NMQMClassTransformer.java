...
public class NMQMClassTransformer implements IClassTransformer {
    ...
    private byte[] patchContainer(byte[] basicClass) {		
	    ...
	    containerClassName = NMQMCorePlugin.isObfuscated() ? "xi" : "net/minecraft/inventory/Container",
	    itemStackClassName = NMQMCorePlugin.isObfuscated() ? "zx" : "net/minecraft/item/ItemStack",
	    entityPlayerClassName = NMQMCorePlugin.isObfuscated() ? "wn" : "net/minecraft/entity/player/EntityPlayer";	 		    
	...
	private byte[] patchGuiContainer(byte[] basicClass) {        
        ...
        classReader.accept(classNode, 0);
	 	...
	 	slotClassName = NMQMCorePlugin.isObfuscated() ? "yg" : "net/minecraft/inventory/Slot";  	 	
        boolean isSuccessful = false;        
        int aloadCount = 0;       
        AbstractInsnNode currentInsn;
	...
	private byte[] patchClickWindowPacket(byte[] basicClass) {		
	    ...
	    iNetHandlerPlayServerClassName = NMQMCorePlugin.isObfuscated() ? "ic" : "net/minecraft/network/play/INetHandlerPlayServer",
	    cPacketClickWindowClassName = NMQMCorePlugin.isObfuscated() ? "ik" : "net/minecraft/network/play/client/C0EPacketClickWindow";	 		    
        boolean isSuccessful = false;       
        AbstractInsnNode currentInsn;
