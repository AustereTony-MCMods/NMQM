...
public class NMQMClassTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {   
        ...
        case "net.minecraft.network.play.client.CPacketClickWindow":
            return patchCPacketClickWindow(basicClass);
        }       
        return basicClass;
    }
    ...
    private byte[] patchContainer(byte[] basicClass) {
        ...
        slotClickMethodName = NMQMCorePlugin.isObfuscated() ? "a" : "slotClick",
                containerClassName = NMQMCorePlugin.isObfuscated() ? "abd" : "net/minecraft/inventory/Container",
                        itemStackClassName = NMQMCorePlugin.isObfuscated() ? "adz" : "net/minecraft/item/ItemStack",
                                clickTypeClassName = NMQMCorePlugin.isObfuscated() ? "abi" : "net/minecraft/inventory/ClickType",
                                        entityPlayerClassName = NMQMCorePlugin.isObfuscated() ? "zs" : "net/minecraft/entity/player/EntityPlayer";                      
    ...
    private byte[] patchGuiContainer(byte[] basicClass) {
        ...
        handleMouseClickMethodName = NMQMCorePlugin.isObfuscated() ? "a" : "handleMouseClick",
                clickTypeClassName = NMQMCorePlugin.isObfuscated() ? "abi" : "net/minecraft/inventory/ClickType",
                        slotClassName = NMQMCorePlugin.isObfuscated() ? "acc" : "net/minecraft/inventory/Slot";                 
        boolean isSuccessful = false;        
        int aloadCount = 0;       
        AbstractInsnNode currentInsn;
    ...
    private byte[] patchCPacketClickWindow(byte[] basicClass) {
        ClassNode classNode = new ClassNode();
        ClassReader classReader = new ClassReader(basicClass);
        classReader.accept(classNode, 0);
        ...
        modeFieldName = NMQMCorePlugin.isObfuscated() ? "f" : "mode",
                processPacketMethodName = NMQMCorePlugin.isObfuscated() ? "a" : "processPacket",
                        clickTypeClassName = NMQMCorePlugin.isObfuscated() ? "abi" : "net/minecraft/inventory/ClickType",
                                iNetHandlerPlayServerClassName = NMQMCorePlugin.isObfuscated() ? "ij" : "net/minecraft/network/play/INetHandlerPlayServer",
                                        cPacketClickWindowClassName = NMQMCorePlugin.isObfuscated() ? "ir" : "net/minecraft/network/play/client/CPacketClickWindow";                            
        boolean isSuccessful = false;        
        AbstractInsnNode currentInsn;
